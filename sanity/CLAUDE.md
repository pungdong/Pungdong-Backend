...
- `schemas/venue.ts` — **공식(OFFICIAL) 위치(수영장/딥풀)**. 어드민 authoring, 강사 코스 빌더가 참조하는 정적 카탈로그. 이용권(평일/주말·고정/상시·키반납)·정기휴무·사진/영상·**기본 대여 장비**(`defaultEquipment[]` = `venueEquipDefault` object: name·price·sizeFormat — 강사 가격표 prefill 전용, `equipInfo` 는 자유 텍스트 비고로 강등)·장비비고. `tickets[].disciplines` = BE `discipline.code` soft-ref. 시간은 `"HH:mm"` 문자열. 오브젝트 타입(`venueTicket`/`venueDaypart`/`venueTimeBlock`/`venueClosure`/`venueEquipDefault`)도 같이 등록. `humanVerified` = 운영자 수기 검수 플래그(Studio 전용, GROQ 미투영·FE/BE 미소비 — seed `--replace` 가 풀어버리니 seed/README.md 백업 절차 참조). **강사 커스텀 위치는 Sanity 아님 — BE DB**(`venue` 도메인). 도메인/정책은 [../docs/features/venue.md](../docs/features/venue.md).
...
- `venue.defaultEquipment[]`(name·price·sizeFormat) ↔ **BE `GET /venue-equipment` prefill**(`VenueEquipmentService` 가 저장분 없는 OFFICIAL 에 `source=VENUE_DEFAULT` 합성, `HttpSanityVenueClient.ALL_QUERY` projection) ↔ **`types.ts`** `VenueEquipmentResponse.source`. `sizeFormat` 값은 BE `SizeFormat` enum(NONE/SHOE_MM/APPAREL_SXL)과 1:1 — BE 는 미상 값을 null 로 lenient 파싱하지만 새 형식 추가 시 세 곳(스키마 options·BE enum·types.ts) 같이. **projection 을 넓히면 `OfficialVenueCache.SCHEMA_VERSION` 도 +1**(구형 캐시 lazy reload).
