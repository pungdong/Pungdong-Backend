...
- `venue.tickets[].disciplines` / `venue.type`(SHALLOW_POOL/DEEP_POOL/OCEAN) / `maxDepth`(최대수심) / `address`(도로명)·`addressDetail`(세부) / daypart·closure 모양 ↔ **venue 도메인**([../docs/features/venue.md](../docs/features/venue.md), [../docs/architecture/venue.md](../docs/architecture/venue.md), BE `VenueResponse` 통합 응답). 미래 BE 동기화 시 `venueRevs`(`_rev` 대조) 사용.
