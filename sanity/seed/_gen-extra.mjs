// venues-extra.ndjson 생성기. 조사 결과(deep-research + enrich + 2026-08-11 Track B 전면 재검증)를
...
// 6. 시흥 파라다이브35 — 35m 딥풀, 기본 장비 무료 대여(확인 일치).
push({
  ...
  equipInfo: '2023.10 개장 · 국내 최심급(35m)\n마스크/스노클/핀/웨이트/슈트 무료 대여 · 카본핀 10,000원(재확인 필요)\n준회원/정회원 멤버십 할인 별도\n문의 031-497-3133',
  defaultEquipment: [
    equip('pd-e1', '마스크', 0, 'NONE'),
    equip('pd-e2', '스노클', 0, 'NONE'),
    equip('pd-e3', '핀', 0, 'SHOE_MM'),
    equip('pd-e4', '웨이트', 0, 'NONE'),
    equip('pd-e5', '슈트', 0, 'APPAREL_SXL'),
  ],
  closures: [],
  tickets: [
...
// 13. 뉴서울다이빙풀(광명) — SSI/NAUI 트레이닝. 최근 활동 근거 약함(비고).
push({
...
// 14. 다이브라이프(서울 서초) — 기존 입장료 25,000원은 근거 미확인으로 제거(갭필 판정), 요금 미공개 처리.
push({
  ...
  equipInfo: '프리다이빙·스쿠버다이빙·인어(머메이드) 강습 자체 풀 · 네이버 예약제(1인 2시간)\n입장료 공식 미공개 — 예약 시 확인(체험 프리다이빙 40,000원~ 상품 별도, 시설이용·장비대여료 별도 안내)\n장비 대여 품목·가격 미공개',
  closures: [],
  tickets: [
    ticket('dl-t1', '일반권', [FD, SC, MM], open(undefined, '10:00', '22:00'), same(undefined)),
  ],
})
