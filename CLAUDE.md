...
> **트랜잭션·격리 규약은 [docs/architecture/transactions.md](docs/architecture/transactions.md).** MySQL 기본 REPEATABLE READ 에선 트랜잭션이 첫 SELECT 시점 스냅샷을 끝까지 본다 — `REQUIRES_NEW` 로 방금 커밋한 행조차 바깥 plain SELECT/COUNT 엔 안 보인다(enrollment H-4 · payment 원장 · community 리액션 count, 세 번 밟음). "쓰고 → 바로 세서 → 응답" 경로는 그 문서의 체크리스트(REQUIRES_NEW 읽기 / 잠금 읽기)를 따르고, **H2 기본(READ COMMITTED)으로 초록인 건 증명이 아니다.**
...
### Validate input shape — `@NotBlank` is not enough

**Repo rule (set 2026-07-11):** every request DTO field is validated for its **actual shape**, not just presence. `@NotBlank`/`@NotNull` only proves *something* arrived — a field that has a format (phone, birth date, OTP, email, 우편번호, 사업자번호, enum-ish string) also carries `@Pattern`/`@Size`/range. **Why it bites:** a shape-less field flows downstream and fails where it's expensive — a malformed value hits a **paid external API** (다날 본인확인 문자), gets silently mangled by a parser (`toIsoDate` passed a non-8-digit string straight through), or is stored and later trusted (age check on a birth date). FE validation is UX, not a boundary — console/proxy/direct calls bypass it, so BE must be the wall. The identity-verification DTOs had only `@NotBlank` and shipped exactly these three holes (fixed in PR #172).

- **Put the check on the DTO (`@Valid`), not deep in the service** — Bean Validation runs at controller entry, *before* side-effecting work (a Redis cooldown slot, a DB write, an external call). One typo then can't burn the user's send-cooldown or touch a paid API. (identity-verification: `@Valid` fires before `acquireSendSlot`.)
- **Normalize before validating** when clients legitimately send separators — a setter strips them (`010-1234-5678` → `01012345678`) so the canonical form is what's validated, stored, and sent onward. Don't reject a shape the client already sends unless you mean to break the contract.
- **Return which field failed, and why.** Format errors surface the field's validation message in the response `msg` (`throw new BadRequestException(result.getFieldError().getDefaultMessage())` — `ExceptionAdvice.badRequest` already forwards a non-null message). **Why this is safe to expose (and login errors aren't):** hiding "which field" only buys security when the response would leak a *server secret* the attacker can't otherwise know — login fuses "no such account"/"wrong password" to stop **user enumeration** (an oracle over DB state). A **format** rule is not an oracle: it's already public (in `types.ts`, in the FE), and the answer doesn't depend on server state (same input → same verdict regardless of what's in the DB). Nothing to hide → take the usability. (This repo already *accepts* the stronger enumeration exposure — `check/email` returns `{exists}` on purpose, since a user must know *what* to fix.) The mirror rule: a form field that checks **secret/stateful** existence (invite code, coupon, "account exists") *is* an oracle — there, weigh enumeration/brute-force before revealing, and lean on rate-limit + attempt caps.
- **Format-error message must be user-facing Korean** if FE renders it verbatim (the identity-verification `@Pattern` messages are). Not every `BadRequestException` message is safe to show, so only greenlight "display as-is" for messages written for users.
- **Claude must apply this proactively:** when adding or reviewing any request DTO, if a field with a real shape carries only `@NotBlank`/`@NotNull`, add the format constraint — even if not asked. Flag every field whose value is parsed, sent to an external service, or persisted-then-trusted.
