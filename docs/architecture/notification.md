...
flowchart TB
    ...
    subgraph Worker["notification 도메인 — 발송 측 (별도 트랜잭션)"]
        Dispatcher["NotificationDispatcher<br/>@Scheduled(fixedDelay=10s)<br/>@Profile('!test')"]
...
sequenceDiagram
    ...
    Note over Dis: @Scheduled(fixedDelay=10s)<br/>@Profile('!test')
...
**발송 메시지 구성 (카테고리 → 채널/priority)**: 알림은 `NotificationType.getCategory()`(`NotificationCategory`)를 가지며, 워커가 그 카테고리를 게이트웨이에 넘긴다. `FirebaseFcmGateway` 가 `AndroidConfig` 에 **`channelId`**(앱이 만든 채널로 라우팅) + **`priority`**(거래성=HIGH 절전회피·즉시, 공지/마케팅=NORMAL)를 실음. 채널 importance/소리/유저토글은 앱 소유. iOS `interruptionLevel` 은 iOS 활성화 때. 채널표·정책은 [features/push.md §채널/카테고리](../features/push.md).
