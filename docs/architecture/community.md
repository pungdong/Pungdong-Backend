...
erDiagram
  ...
  BrandingPost {
    enum category "TOUR|TRAINING|MATCH|QNA, NOT NULL (V31)"
    String title "100, NOT NULL (V31)"
...
- **`category`·`title` 은 V31 부터 NOT NULL 이다**(기존 행은 `TOUR` / `'브랜딩 게시물'` 로 backfill). 두 쓰기 경로 모두 필수로 받는다. 카테고리 없는 글을 남겨두면 **수정하려는 작성자가 없던 분류를 발명해야** 한다. backfill 이 "없는 값을 지어내지 않는다" 와 충돌하지 않는 이유는 대상이 실사용자 데이터가 아니어서다 — prod 최종 배포(a383968)에는 V17·V19 가 없어 이 테이블 자체가 존재하지 않는다. 제목도 같은 이유로 함께 조였다 — 통합 폼이 제목을 필수로 받으므로 제목 없는 옛 글은 수정 자체가 막힌다.
- **`show_on_profile` 은 이제 작성자가 요청으로 고른다**(`CommunityPostRequest.showOnProfile`, 기본 false). V19~2026-08-18 에는 *작성 경로*가 정했다(브랜딩=true / 커뮤니티=false). `show_in_feed` 는 두 경로 모두 항상 1 이고, V31 에서 남아 있던 0 행(구 브랜딩 글)도 1 로 올렸다 — 통합 모델에는 "숨기지도 않았는데 피드에 없는 글"을 만드는 쓰기 경로가 없어, 남겨두면 화면에서 설명되지 않는 세 번째 축이 된다. 컬럼은 유지한다(피드 인덱스 선두 컬럼이고 "프로필 전용" 이 필요해지면 쓸 자리).
...
**보내지 않은 키는 "변경 없음" 이 아니라 "비우기" 다.** `mediaUrls`·`tags` 는 DTO 가 빈 배열로 초기화돼 있어
키를 생략하면 `@NotNull` 을 통과한 뒤 **전삭제**로 해석되고, 빠진 사진은 **S3 에서 즉시 영구 삭제**된다.
...
모집글은 제목 오타조차 못 고쳤다**(프리필한 과거 날짜를 되돌려 보내면 400). 규칙의 의도는 "과거 날짜로
*모집하지* 마라" 지 "과거 모집글을 *손대지* 마라" 가 아니다.

**🟡 `updatedAt` 은 "수정됨" 표기의 근거로 쓸 수 없다(백로그).** 양방향으로 틀린다 — 사진·태그만 바꾸면
부모 행이 dirty 가 아니라 `@PreUpdate` 가 안 돌아 **안 움직이고**(테스트 `E3` 가 `@Disabled` 로 기록),
숨김 토글은 부모를 dirty 하게 만들어 본문을 안 고쳤는데도 **움직인다**. 표기를 켜려면 이 둘을 먼저
고치거나 `editedAt` 을 따로 둬야 한다. 현재는 표기하지 않기로 결정돼 있다.
...
- 🟢 **DRAFT 코스 연결은 "거르지 않고 두 번 고지" 로 닫혔다.** 요청은 200 으로 통과하고 **공개** 응답에서 `linkedCourse` 키만 빠지며, 코스를 OPEN 으로 바꾸면 그때 나타난다("준비 중인 강의를 미리 걸어둔다" 는 유효한 사용이라 서버는 막지 않는다). 침묵만 없애면 되므로 **FE 가 선택 시점(옵션 뱃지/라벨)과 선택 후(폼 안내) 두 번** 알린다 — 시트·드롭다운은 닫히면 안 읽히기 때문이다.
  **단 오너 본인의 상세에는 DRAFT 도 실린다**(2026-08-18). 공개 규칙을 오너에게까지 적용했더니 수정 폼이 `linkedCourseId` 를 프리필하지 못해 **저장하는 순간 연결이 조용히 끊겼다** — 우리가 권장하는 사용을 우리가 파괴하는 경로였고, 요청에서 "사용자가 뗐다" 와 구별되지 않아 클라이언트가 방어할 수도 없었다. 피드 카드는 오너에게도 공개 규칙 그대로다(카드엔 오너 개념이 없다).
...
- `G1~G10` **Phase 5 독립 리뷰에서 잡힌 것들** — 댓글·대댓글·댓글좋아요 달린 글 삭제(커뮤니티·브랜딩 **양 경로**) / 강사 글 필터(전용 쿼리 포함) / 조치된 글 재공개 차단 / 커뮤니티 글이 브랜딩 수정·상세 경로로 새지 않는지 / 숨긴 글의 댓글 좋아요 차단 / `mediaUrls: null` 이 500 아닌 400 / 어드민 큐 size 상한
