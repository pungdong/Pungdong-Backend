...
- 🟡 **강의 연결의 "강사만" 은 커뮤니티가 보장하지 않는다.** 서비스가 보는 건 **코스 소유권 하나**이고, `POST /courses` 는 `authenticated()` 라 STUDENT 롤 계정도 코스를 만들 수 있다. 즉 작성자 칩은 `isInstructor: false` 인데 강의 미니카드가 붙는 상태가 가능하다(판정 축이 둘 — 칩은 *승인된 신청*, 연결은 *코스 소유*). → 좁히려면 코스 도메인의 결정이 먼저다.
