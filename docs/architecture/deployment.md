...
## 9-1. 조회 조건을 좁히는 배포는 "기준선 → 대조" 로 검증한다 (2026-08-22)

필터를 추가하는 배포(예: "승인된 강사의 강의만 노출")는 **성공해도 조용히 데이터를 가릴 수 있다.** 헬스도 초록이고 digest 도 맞는데 목록만 비는 식이라, **배포 후 응답만 봐서는 정상인지 알 수 없다** — 비교 대상이 없기 때문이다.

절차는 단순하다:

1. **배포 전** 공개 API 로 기준선을 찍는다 — `GET /courses/browse?disciplineCode=…&size=50` 의 `page.totalElements` 와 목록.
2. 배포.
3. **같은 호출로 대조.** 숫자가 그대로여야 한다.

2026-08-22 강사 승인 게이트 배포에서 이렇게 확인했다(prod FREEDIVING 5 · SCUBA 2 → 배포 후 동일). DB 접근 없이 되고, private RDS 라 one-off Fargate task 를 띄우지 않아도 된다는 게 실용적인 이점이다.

**승인 같은 파생 상태는 그 상태만 보는 엔드포인트로 확인한다.** 위 배포 전에 `GET /instructors/{nickName}` 의 `isInstructor` 로 승인 여부를 판정하려다 **6명이 400 이라 "강의가 사라진다" 고 오판할 뻔했다** — 그 엔드포인트는 당시 prod 코드(#302 이전)에서 **브랜딩 미발행만으로도 400** 이었다. 승인만 보는 `GET /instructors/public`(브랜딩과 무관)으로 바꾸니 전원 승인·종목 일치였다.

> ⚠️ **교훈**: *같은 배포에서 의미가 바뀌는 엔드포인트를 프로브로 쓰지 말 것.* 프로브는 **이번 배포가 건드리지 않는** 표면이어야 한다.
