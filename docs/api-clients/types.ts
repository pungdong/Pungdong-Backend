...
export interface IdentityVerificationRequest {
  ...
  gender: Gender;
  /**
   * 주민등록번호 7번째 자리(성별식별자, '1'~'8') — 선택이지만 **FE 는 항상 보낼 것**.
   * 다날 전송용 identityNumber(주민번호 앞 7자리)의 원본. 생략 시(구버전 앱) 서버가
   * birth+gender 로 역산하는데 내국인(1~4)만 무손실 — 외국인(5~8)은 원본 없이는 인증 실패.
   * 형식 위반 → 400.
   */
  rrnSeventhDigit?: string;
  /**
 ...
 * 환불율(CONFIRMED 회차만) = 당일0/전날50/2일전70/3일전+100, 단 강사 수락(확정) 후 1h 내 100(그레이스). 미수락(ACCEPT_PENDING) 회차는 날짜 무관 100.
 * 수강료는 1회차에 전액 냈으므로 1회차 결제주문 부분취소.
 */
