...
### ⚠️ 업로드 제약 (2026-08-10 추가) — 공개 이미지 3종 공통

프로필 사진 · 코스 이미지 · 리뷰 이미지 업로드에 **서버 검증이 생겼다.** 어기면 **400 + 사용자 문구**가 `msg` 로 내려오니 그대로 노출하면 된다.

| 규칙 | 값 | 위반 시 `msg` |
|---|---|---|
| 형식 | `image/jpeg` · `image/jpg` · `image/png` · `image/webp` **만** | "지원하지 않는 이미지 형식이에요. JPG · PNG · WEBP 만 올릴 수 있어요." |
| 크기 | **8MB 이하** | "이미지가 너무 커요. 8MB 이하로 올려주세요." |
| 빈 파일 | 거부 | "이미지를 선택해주세요." |

- **`Content-Type` 을 반드시 실어 보낼 것** — 없으면 거부한다(검증할 수 없는 값을 통과시키지 않는다). 대소문자·`; charset=` 파라미터는 서버가 정규화한다.
- **HEIC 는 미지원** — iOS 에서 원본 그대로 올리면 거부된다. 피커 단계에서 **JPEG 로 변환**해 보내라.
- 리뷰 이미지는 다중 업로드인데 **한 장이라도 어긋나면 전량 거부**한다(절반만 저장되지 않도록).
- 8MB 는 임의 숫자가 아니다 — 변환 Lambda 가 원본을 메모리에 올린 뒤 결과를 base64 로 반환해서 페이로드 상한(~6MB)에 걸린다. **그래서 `/r/` 는 항상 `w`·`fm` 을 붙여 호출해야 한다**(아래). 파라미터 없이 `/r/{key}` 를 부르면 원본이 base64 로 실려 나가 큰 파일에서 실패한다.
