...
> ✅ 아래 계약은 **구현 완료**(푸시 v2 — `DeviceController` + `data.notificationId`). 앱 v2 가 이 계약에 맞춘다.
...
- **구현**: `account/DeviceController` (`POST /me/devices` 200, `DELETE /me/devices/{token}` 204). 옛 `POST /sign/firebase-token` 은 제거. 인증=`anyRequest().authenticated()`.
- **왜 리네임**: 옛 경로는 `/sign`(auth 네임스페이스) 아래 + `firebase`(벤더명)을 URL 에 박음 — 디바이스 등록은 "sign" 작업도 아니고, 공급자(웹푸시 추가/교체) 바뀌면 경로가 거짓말이 된다. 리소스는 *내 디바이스들*(`/me/devices`)이어야 함.
- **platform 은 선택**: 보내면 `deviceType` 저장, 없으면 null(엔티티 컬럼 nullable). (`model` 은 안 받음.)
...
{ "notification": { "title": "...", "body": "..." },
  "data": {
    "notificationId": "<uuid>",        // at-least-once dedup 키 (아래 §정책)
    "type": "RESERVATION_CREATED",     // 앱이 라우팅 분기
    "lectureId": "123", "scheduleId": "456"
  } }
```

- **구현**: `NotificationOutboxWriter.enqueue` 가 모든 이벤트에 `notificationId`(UUID)를 주입. 메커니즘은 [notification.md §데이터 모델](../architecture/notification.md).
- 앱은 `data.type` 으로 deep-link 화면을 고르고 `notificationId` 로 dedup 한다. **앱이 키를 임의로 요구하는 게 아니라 BE 스키마를 따른다.**

### 3. 자격증명 — WIF 키리스 (JSON 키 금지)

- prod: AWS ECS task role ↔ `plop-5997b` SA via **Workload Identity Federation**, 키 파일 0. (⚠️ Fargate 라 google-auth 내장 AWS 공급기가 task role 을 못 읽어 AWS SDK 기반 supplier shim 한 조각 필요 — PR #134. "코드 변경 0"은 EC2 가정이었고 Fargate 엔 안 맞았음.)
- ⚠️ **WIF 자격엔 project id 가 없다** → Admin SDK 가 FCM 엔드포인트(`/v1/projects/<id>/messages:send`)를 못 만들어 `Project ID is required` 로 실패. 표준 env **`GOOGLE_CLOUD_PROJECT=plop-5997b`** 로 명시(PR #136, staging 검증서 발견). service account JSON 엔 들어있어 이 경로에선 불필요.
...
### 4. 채널 / 카테고리 (FE 협의 2026-06-30, 구현)

알림은 **카테고리**(`NotificationCategory`)를 가지고, 카테고리가 **Android channelId + 발송 priority + 마케팅 야간제한 여부**를 결정한다. NotificationType → 카테고리 매핑은 BE 소유.

| channelId | 이름(유저 노출) | Android 중요도(앱 소유) | priority(BE, delivery) | 성격 | 야간제한 |
|---|---|---|---|---|---|
| `reservation` | 예약 알림 | HIGH | HIGH | 거래 | — |
| `payment` | 결제 알림 | HIGH | HIGH | 거래 | — |
| `chat` | 채팅 | HIGH | HIGH | 거래 | — |
| `notice` | 서비스 공지 | DEFAULT | NORMAL | 약관·개인정보 등 필수 고지(마케팅 아님) | — |
...
- **iOS 는 채널 개념 없음** — 앱당 알림 권한 하나뿐. 카테고리별 opt-out(마케팅만 끄기)을 OS 가 못 줘서 → **인앱 마케팅 동의 토글 + BE 게이팅**이 그 역할(크로스플랫폼으로 이게 더 중요). prominence 는 선택적 APNs `interruptionLevel`(거래=time-sensitive / 마케팅=passive, 발송측 지정)로 — iOS 활성화 때.
...
- ⚠️ 현재 NotificationType: 예약 2종→`reservation`, `LECTURE_NOTIFICATION`→`notice`(정보성 가정). **강사 알림이 홍보성이면 `marketing` 재분류 필요**(야간제한+동의). payment/chat/marketing 타입은 해당 피처 붙을 때 추가.
...
| 2026-06-29 | WIF 구축 — GCP 신뢰 설정(gcloud) 완료 + Fargate 용 supplier shim. "코드 변경 0" 가정은 Fargate 에서 틀림(google-auth 가 컨테이너 자격 엔드포인트 미지원) | [PR #134](https://github.com/pungdong/Pungdong-Backend/pull/134) |
...
- 🟡 **iOS `interruptionLevel`** — `ApnsConfig`(거래=time-sensitive/마케팅=passive). iOS 활성화(법인 Apple+APNs) 때.
- 🟡 **`LECTURE_NOTIFICATION` 카테고리 재확인** — 현재 `notice`. 홍보성이면 `marketing` 으로(야간제한+동의).
