...
- 🟢 **WIF 신뢰 설정** — GCP 측(pool `aws-pool`/`aws-provider`/SA `workloadIdentityUser` 바인딩 staging·prod) gcloud 로 완료, BE 코드 shim [PR #134](https://github.com/pungdong/Pungdong-Backend/pull/134). 🔴 **남은 건 staging `terraform apply`+force-new-deploy 후 실송신 검증 → prod env 동일 적용** = 실송신의 마지막 한 칸.
