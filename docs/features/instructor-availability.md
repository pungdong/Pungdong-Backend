...
- 디자인 원칙: "**사실은 보여주고, 판단은 강사가**" — 자동 경고·임의 임계값 없음. 정원 초과를 막지 않고 보여주기만 한다. (시간이 **겹치는 session 만은 예외로 거부** — 한 강사는 한 번에 한 세션이라 이중부킹·정원 이중계산이 되는 구조적 오류다. `-1015` 응답에 겹친 일정 목록을 실어 강사가 무엇을 비워야 하는지 보여준다.)
- **하루 끝의 정식 표현 = `23:59:59`** (2026-08-22). `LocalTime` 은 24:00 을 표현할 수 없어 타임라인 끝까지 끈 `"24:00"` 은 400 이고, FE 는 `"23:59:59"` 를 보낸다. BE 는 **23:59 이상으로 끝나는 값을 전부 `23:59:59` 로 정규화**해 저장·응답한다 — coverage·session·커스텀 위치 운영블록 공통(`availability/DayEnd`). 이유: 수강신청 슬롯은 venue 블록이 coverage 에 *통째로* 들어갈 때만 생기므로, 끝 표현이 `23:59:00`/`23:59:59` 로 섞이면 하루 끝까지 연 강사의 늦은 블록이 59초 차이로 조용히 탈락한다. OFFICIAL 위치는 Sanity 스키마가 `HH:mm`(최대 23:59)이라 항상 그 이하 → 포함 성립. `"24:00"` 을 받아 정규화하는 안(B)은 모든 `LocalTime` 필드에 걸치는 전역 Jackson 모듈이 필요해 보류.

### coverage 편집 vs 일정 추가 — 두 진입점

- **예약가능시간 직접 편집**: 열기(`POST /coverage`, union·머지, recurrence ONCE/WEEKLY/MONTH 로 여러 날 × `timeRanges[]` 로 여러 시간대를 한 번에)·닫기(`DELETE /coverage`, subtract, 단일 날). "예약 받을 시간"만 손대는 가벼운 도구.

#### 반복 전개 — "어느 기간" 이지 "언제부터" 가 아니다 (2026-08-23)

강사가 기대하는 건 4주가 아니라 **달력 월 전체**라 `FOUR_WEEKS` 는 달 끝을 조용히 빠뜨렸다(9월 화요일 기준 **9/29 누락**). `MONTH` 로 대체했고 `FOUR_WEEKS` 는 구 클라이언트 하위호환으로만 남는다.

같은 함정이 **시작점**에도 있었다. 요청의 `date` 를 전개 시작점으로 쓰면 강사가 달·주 중간 날을 찍었을 때 기간 앞부분이 빠진다 — `9/15` 를 찍고 "이 달 반복·화" 를 누르면 **9/1·9/8 이 사라지고**, `9/2` 를 찍고 "이 주 반복·월" 을 누르면 아직 미래인 **8/31 이 탈락**한다. 그래서 반복 모드의 시작점은 `max(오늘, 기간 시작)` 이다(MONTH=그 달 1일, WEEKLY·FOUR_WEEKS=그 주 월요일 ISO). **실패가 에러가 아니라 "덜 열림" 이라 더 나쁘다** — 강사는 열었다고 믿고 넘어간다.

FE 도 요청 전에 같은 정규화를 걸지만(#766), 서버에도 두는 이유는 규칙이 클라이언트 코드에만 살면 `types.ts` 의 "MONTH = 그 달 전체" 가 직접 호출자에게 거짓이 되기 때문. 이미 정규화된 값에는 no-op(고정점)이다. **과거는 오늘로 clamp 되어 어떤 반복 경로로도 안 열린다.**

전개 결과가 **0일이면 200 + 빈 배열**이다. 예전엔 400 을 던졌는데 시간 형식 오류와 응답이 완전히 같아(`-1011`) FE 가 구분할 수 없었다 — "선택한 요일이 이 기간에 안 남았다" 는 정상적으로 계산된 답이다. FE 는 저장 버튼 비활성으로 애초에 막고, BE 는 뚫고 들어온 요청에 조용한 no-op 으로 답한다.

`timeRanges[]` 는 점심을 비우고 오전·오후를 따로 여는 흔한 케이스를 **한 트랜잭션**으로 받기 위한 것(여러 번 POST 하면 앞의 것만 반영되는 부분 실패가 난다). 첫 구간은 `startTime`/`endTime` 에도 겹쳐 보내서, 이 필드를 모르는 구 서버에 닿아도 "조용히 틀림" 이 아니라 "첫 구간만 열림" 으로 degrade 한다.
