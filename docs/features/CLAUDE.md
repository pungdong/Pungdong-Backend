...
- [검색 색인·발견(seo-indexing)](seo-indexing.md) — course(공개 상세 읽기 게이트·`published_at`) + branding + community · **웹 URL 은 판매 화면이기 전에 색인 자산** · 읽기 축 ≠ 행동 축(마감 강의는 읽히지만 저장 400) · 판정은 상태가 아니라 발행 이력 · `lastmod` 는 모르면 안 낸다(무엇을 못 잡는지 표) · sitemap 에 마감분은 안 싣는다 · FE 짝 문서 `PungDong/docs/web-seo.md`
