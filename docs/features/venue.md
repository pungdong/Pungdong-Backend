...
프리/스쿠버 강의는 강의가 중심이 아니라 **장소(수영장·딥풀·해양 포인트) = Venue** 가 중심이다. 입장료·운영 시간대·이용 옵션·정기 휴무·장비 대여처럼 **장소에 종속되는 정보를 강의에서 풀지 않고 Venue 에 모은다**. 강사는 코스를 만들 때 위치를 고르기만 하면 그 Venue 가 제공하는 시간대가 따라온다.
...
- 이걸 **강의에서 전부 풀려고 하면 너무 복잡하고 어려워진다.** → 그래서 **Venue 도메인을 정의하고, 수영장에 종속되는 대부분의 내용(시간대·휴무·입장료·이용 옵션·장비)을 전부 Venue 에 종속**시킨다. 코스는 그걸 **참조만** 한다.
...
| discipline | [discipline.md](../architecture/discipline.md) | 이용 옵션 대상 종목 코드 검증·soft-ref |
