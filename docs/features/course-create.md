...
강사가 강의 상품(코스)을 만드는 화면(모바일 3-step 위저드 / 데스크탑 동일 3-step)의 BE. **기본정보**(단체·레벨·수강료·사진) + **회차**(회차별 설명·진행 위치·이용권) + 선택 **추가세션**(비용 정책). V2 의 Course 는 legacy `Lecture` 의 후신이며 **legacy 쪽은 2026-08-15 삭제됐다** — 장소 종속 정보(위치·입장료·장비)를 강의에서 풀던 옛 모델을 버리고, **위치/장비는 venue 도메인이 소유하고 코스는 참조만** 한다.
...
- **종류는 상호배타 (UI 평탄화 X, 결정 2026-06-14)** — 체험/트레이닝은 자격단체에 종속되지 않고 자격증만 종속되는 비대칭이라, 한 셀렉터에 평탄화하지 않는다. FE 도 모델과 동일하게 종류를 상호배타로 두고, CERTIFICATION 일 때만 단체→자격증을 노출. 강사 멘탈모델 = "이 강의로 자격증을 발급하나?"(아니오=체험 / 예,X레벨=자격증 / 아니오,보충=트레이닝).
  - "묶음"처럼 보이는 욕구는 레벨에 섞지 않고 각자 메커니즘으로: **패키지** = 자격증 멀티선택(아래) · **트레이닝 포함** = 추가세션(무료 N회) + 강의상세 어필 · **체험 프레이밍**(자격과정인데 입문 느낌) = 제목·설명(반복 수요면 kind 와 직교한 태그/뱃지가 릴리스밸브, 지금은 불필요).
- 레벨은 **평탄화**(LEVEL_1~4/INSTRUCTOR) — 단체마다 명칭(예: "Advanced Freediver")은 달라도 공통 사다리. 명칭은 Sanity, BE 는 코드만. (※ "레벨 명칭 평탄화"와 위 "종류 UI 평탄화 안 함"은 다른 얘기 — 전자는 단체별 명칭 정규화, 후자는 체험/자격/트레이닝을 한 줄에 안 섞음.)
...
### 할인 (방향 합의 · 구현 보류, 2026-06-14)
> 구현 안 됨. 착수 시 이 방향에서 시작 — 대화로만 남기지 않으려 박제.

- **할인은 Course(수강료)에 붙는다.** 비용 3종 중 성격이 다름:
  - **수강료** = 강사 매출(강의 자체 값) → **할인 대상**.
  - **입장료/이용권** = 시설 pass-through 비용 → **할인 불가**(강사·플랫폼이 시설 돈을 임의로 못 내림).
  - **장비 대여료**(equipment extension) = 강사가 정하는 단가 → 낮출 순 있으나 그건 "할인"이 아니라 **단가 설정**(별도 할인 construct 불필요).
- **정산 정합**: 수수료(PG 3.4% + 플랫폼 6.6%)는 수강료에만(부대비용 0%). 할인을 수강료에만 걸면 base·강사 net 이 자연히 같이 내려가고 입장료·장비는 손 안 탐 → "정직한 비용 분해"와 일관.
- **MVP 방향(추천)**: 강사 세일가(정가 + 할인율/금액, 선택적 기간), **강사 부담**. 쿠폰/프로모코드·플랫폼 보전 할인은 **별도 도메인 후속**.
- **착수 시 결정할 것**: 입력 기준(할인율 % vs 금액 원), 기간 한정 포함 여부, 모델 위치(Course 필드 vs 별도 `CourseDiscount`). 패키지는 강사가 묶음가 직접 정하니 별도 할인 메커니즘 불필요(표시만 선택).
...
- ~~🟢 **legacy `Lecture` → Course 마이그레이션**~~ — ✅ 해소(2026-08-15, legacy 삭제).
