...
- **개인정보처리방침 개정** — Sanity(legalDocument)에서 직접 수정. §7(파기)에 30일 유예·복구·익명화·법정보존 분리를 명시, §1 에 프로필사진·푸시토큰 추가, §6 에 탈퇴 경로(설정>회원탈퇴·웹) 명시. **미런칭이라 버전은 v1.0 유지**(고지의무 7일 전 공지 불필요 — 시행 전 수정). (plop.cool/privacy)
...
- **2026-06-29 (정책)** — **개인정보처리방침 문안 개정** (Sanity 직접 수정, 미런칭이라 v1.0 유지). 기존 §7 "지체 없이 파기" 가 30일 유예 보관과 충돌 → 유예·복구·익명화·법정보존 분리를 명시하도록 개정. 코드(실제 동작)와 처리방침(공시)을 정합화.
