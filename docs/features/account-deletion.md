...
- **개인정보처리방침 v1.1 반영 완료** (2026-06-29 문안 확정) — §7(파기)에 30일 유예·복구·익명화·법정보존 분리를 명시, §1 에 프로필사진·푸시토큰 추가, §6 에 탈퇴 경로(설정>회원탈퇴·웹) 명시. 시행일은 §10 고지의무(7일 전 공지)상 공지일+7일 이후로. (plop.cool/privacy)
...
- **2026-06-29 (정책)** — **개인정보처리방침 v1.1 문안 확정.** 기존 §7 "지체 없이 파기" 가 30일 유예 보관과 충돌 → 유예·복구·익명화·법정보존 분리를 명시하도록 개정. 코드(실제 동작)와 처리방침(공시)을 한 PR 의 결정으로 정합화.
