...
| payment | [payment.md](payment.md) · [architecture](../architecture/payment.md) | (선결제 1회차) 신청 즉시 결제 → ACCEPT_PENDING → 강사 수락 확정 / 거절·만료 자동환불. 정산 후속 |
...
- **만석(신청 시점 좌석 lock · 선착순)** = `활성(대기+결제대기+확정) + 외부 hold >= 유효정원`. **신청과 동시에 좌석을 잡는다**(영화관 좌석식 — 정원 차면 새 신청 400). 정원 ~5명 소규모라 먼저 신청한 사람이 밀리지 않게(2026-06-28 결정 — 옛 "PENDING 하드캡 안 함" 폐기). 수락은 잠긴 슬롯을 확정으로 전환만(정원 재검증 없음). ⚠️ 옛 "결제대기로 전환" 표현은 선결제 전환 이전 것 — `PAYMENT_PENDING` 상태는 2026-08-09 제거됐다(#198/V13).
- **미결제 재신청은 "새 신청"이 아니라 "일정 수정"** (2026-08-11): 결제 화면에서 뒤로 가 날짜를 바꿔 다시 신청하면 옛 미결제 건을 **갈아끼운다**(수강·회차가 하나로 유지되고 옛 좌석은 자동 반납). **왜**: 그냥 새로 만들면 미결제 건이 좌석을 잡은 채 남아 **이중 점유**가 되고, 한정된 정원(~5명)에서 실제 예약을 막는다. 클라이언트가 정리하게 두면 앱 종료·크래시에 반쪽 상태가 남으므로 **BE 가 소유**한다. 스코프는 (학생×강의×미결제)로 좁혀 **결제완료 건은 절대 건드리지 않는다**(그건 강사 SLA 소관).
- **lock 자동 만료(TTL)** — 방치된 점유 해제(선결제): 미결제(PENDING) **12h**(createdAt) 만료(환불 없음) / 결제완료(ACCEPT_PENDING) **24h**(결제시각) 강사 무응답 만료 **+ 전액 자동환불**. **두 갈래뿐**(전 회차 통일). CANCELLED·좌석 해제(`EnrollmentExpiryService` 주기 스위프). 값은 `SiteSettings`(Sanity) **런타임 config**(기본 pending 24/payment 12, Studio 무배포 조정). (만료 푸시 알림은 후속.)
...
- **강사가 코스(틀)를 고쳐도 이미 확정된 예약은 움직이지 않는다** — 회차가 위치·시간을 신청 시점에 스냅샷해 두기 때문. 바뀌는 건 *앞으로 잡을* 회차의 후보다. 강사는 신청 기록이 있는 회차를 **없앨 수는 없고**(400 `-1024`), 그 학생의 그 회차만 조정해야 하면 **회차 채팅**으로 합의한다. 정책은 [course-create.md](course-create.md) "수정 — 수강생이 생긴 뒤에도".

### 선결제 — 신청 즉시 결제 → 강사 수락/거절 (2026-08-07)

- **흐름(전 회차 동일)**: 신청(`PENDING`·미결제·좌석 점유) → **학생 즉시 결제**(`ACCEPT_PENDING`·결제완료·강사 결정 대기) → **강사 수락(`CONFIRMED`)** / 강사 거절·무응답 24h(`REJECTED`·`CANCELLED` + **전액 자동환불**). 미결제 12h 방치는 좌석 해제(환불 없음). **왜**: "신청 따로/결제 따로"는 카드사에 흐름을 설명해야 하는 비표준 → "주문 즉시 결제"가 표준이라 심사 리스크↓ + 어차피 붙일 방향.
- **⛔ 기저 원칙 — 일정 확정에는 강사 수락이 무조건 필요**: 어떤 경로로든(신청·회차 추가·일정 변경·**차액 결제 후 변경**) 일정이 잡히거나 바뀌면 강사가 수락해야 확정된다. **왜**: 강사의 몸은 하나인데 **여러 플랫폼을 동시에 운영**하므로, 우리가 아는 `coverage` 는 강사의 실제 일정을 다 반영하지 못한다 — **강사 일정은 우리에게 미지**이고 확인이 반드시 필요하다. **유일한 예외 = 강사가 먼저 제안한 자리**(`propose-slots` → `pick-slot`): 강사가 가능한 시간을 낸 것 자체가 동의라 추가 수락 없이 확정된다. 결제 여부·금액은 동의의 근거가 아니다.
- **강사 결정은 3지선다** — 수락 / 거절 / **일정조정 제안**. 결제가 앞으로 당겨졌으니 제안도 결제 *후* 시점이 되고, 학생은 결제가 아니라 **ㅇㅋ**(제안 슬롯 선택 → 곧장 확정) / **ㄴㄴ**(취소+전액환불, 또는 내 슬롯으로 재제안)만 한다.
- **옛 "pay-first"(2026-06-28) 재정의**: 그건 *"강사가 수영장을 결제 이후에 예약(풀부킹이 결제 뒤)"*이라는 뜻이었다. 그 통찰(돈 확보 후 풀 잡기, 풀부킹 실패 시 강사 일정변경요청 또는 **전액 무료 환불**·학생 무과실)은 유효하고, 선결제로 결제 시점이 강사 수락보다도 앞(신청 시점)으로 더 당겨진 것.
...
- ~~**거절·수강취소는 진입(1회차) 한정**~~ → **2026-08-09 전 회차로 확대**(선결제 통일). 강사 거절은 이제 어느 회차에서나 가능하고 그 회차 전액 자동환불된다. 위 매트릭스의 "*(거절 없음)*"·"*(수강취소 없음)*" 표기는 그 이전 것이다.

### 취소 / 환불 — 경계는 결제

- **결제 전**(미결제 `PENDING`) = 순수 취소(무료). 강사가 풀을 안 잡아 손해 0. **결제 후·확정 전**(`ACCEPT_PENDING`)도 학생이 취소할 수 있고 **전액 자동환불**된다(강사 제안이 다 안 맞을 때의 "ㄴㄴ").
- **결제 후**(`ACCEPT_PENDING`~) = 취소 = **환불 거래**. 강사 거절·무응답 만료 = **전액 자동환불**(시스템), 강사/venue 사유(풀부킹 실패 등) = 전액 무료, **학생 사유 = 환불정책 페널티**. done 회차 = 0, 미배정 회차 = 수강료÷N 전액.
...
- 회차 상태(`EnrollmentStatus`, **5값 · 전 회차 공통**): `PENDING`(미결제·결제 대기) → `ACCEPT_PENDING`(결제완료·강사 결정 대기) → `CONFIRMED`(강사 수락·확정) / `REJECTED`(강사 거절 — **그 회차만**·자동환불·재신청으로 복구) / `CANCELLED`(취소·만료, 결제완료분 자동환불·재신청 가능). **done = `CONFIRMED && doneAt!=null`**(별도 enum 없음). 강의(수강) 상태는 회차들에서 파생.
- **취소 무료 = 결제 전**(미결제 PENDING). 결제 후 취소는 전액 환불. 거절/취소로 session 활성 신청이 0 이 되면 session 은 잔존(bind/unbind 없음), 이력 스냅샷은 보존.
...
| 2026-06-28 (재설계) | 수강료=1회차 전액·enrollment 스냅샷 / 부대비용=회차별 / 액션매트릭스(거절·취소는 1회차 한정 — 2026-08-09 전 회차로 확대) / 게이트=직전 done(앞당김은 정책완화) / 환불 율(당일0·전날50·2일전70·3일전+100, 1h grace) | 사용자 정책 확정(긴 토의). 환불 실행은 후속 PR, 모델·정책은 지금 박제 |
...
| 2026-08-07 | **선결제 전환(1회차)**: 신청 즉시 결제 → 강사 수락/거절(자동환불), 신규 상태 `ACCEPT_PENDING` | "신청 따로/결제 따로"는 카드사에 흐름 설명해야 하는 비표준 → "주문 즉시 결제" 표준으로 심사 리스크↓ + 어차피 붙일 방향. 거절→자동환불은 검증된 PG cancel 재사용(이벤트→listener). 오버부킹=이중결제라 좌석 하드닝(비관락+세션 UNIQUE V12). 스코프=1회차(2회차+ 구 흐름 유지 — 2026-08-09 통일로 해소). |
...
- 🟡 **다회차 재설계 구현** (2026-06-28~, PR 분할): ✅PR1 엔티티 분할+1회차(pay-first), ✅좌석lock+TTL, ✅PR2+PR3 다회차 진행·완료(2+ 일정신청 `POST /enrollments/{id}/rounds`·`GET .../next-options`·**locked 게이트=직전 회차 done**·강사 **일정변경요청(`propose-slots`→`pick-slot`, 완전한 슬롯[날짜+이용권+블록], 사전수락)**·EXTRA·**완료 done**(강사 `complete`/세션 일괄 `sessions/{id}/complete`/세션일+24h 자동 sweep, hub DONE/COMPLETED 파생)). 남음: PR4 환불 상태기계·정산 연계·다이브로그.
  ...
  - **✅ 직접 일정수정(reschedule, 2026-06-28)** — 학생이 강사 제안 외 **원하는 슬롯**으로 회차를 바꾼다. **취소가 아니라 제자리 수정**(회차 id 유지, 옛 슬롯은 `EnrollmentRound.slotHistory` 적재 — "일정 변경이지 취소가 아님", CS 이력 보존). 날짜 따라 위치가 다를 수 있어 **위치·이용권·장비 재선택** 가능. 강사 미제안 슬롯이라 **강사 재수락이 필요**하다. 흐름: `GET /enrollments/rounds/{id}/options`(그 회차 슬롯, 1회차 옵션 shape) → `POST /enrollments/rounds/{id}/reschedule`. 미결제(`PENDING`)면 그대로 결제 대기, **결제 후(`ACCEPT_PENDING`)면 결제를 유지한 채 강사 결정 대기로 되돌린다**(= 강사 제안이 다 안 맞을 때의 학생 재제안). 금액이 늘면 **차액 결제 경로**로 간다(`-1018` → `POST /payments/prepare` + `target*`) — 단 **위치까지 바꾸며 오르면 `-1019`**(그 경로가 위치를 못 바꿔 취소 후 재신청). 제안 그대로 고르는 빠른 길 = `pick-slot`(추가 결제 없이 즉시 확정) — **단 강사가 더 비싼 시간대를 제안했다면 그것도 `-1018` → 차액 결제**를 거치고, 종착지는 `CONFIRMED` 가 아니라 `ACCEPT_PENDING`(재수락)이다.
