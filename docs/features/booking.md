...
- **lock 자동 만료(TTL)** — 방치된 점유 해제(선결제): 미결제(PENDING) **12h**(createdAt) 만료(환불 없음) / 결제완료(ACCEPT_PENDING) **24h**(결제시각) 강사 무응답 만료 **+ 전액 자동환불** / (2회차) 결제대기(PAYMENT_PENDING) **12h** 미결제 만료. CANCELLED·좌석 해제(`EnrollmentExpiryService` 주기 스위프). 값은 `SiteSettings`(Sanity) **런타임 config**(기본 pending 24/payment 12, Studio 무배포 조정). (만료 푸시 알림은 후속.)
...
- **1회차 흐름**: 신청(`PENDING`·미결제·좌석 점유) → **학생 즉시 결제**(`ACCEPT_PENDING`·결제완료·강사 확인 대기) → **강사 수락(`CONFIRMED`)** / 강사 거절·무응답 24h(`REJECTED`·`CANCELLED` + **전액 자동환불**). 미결제 12h 방치는 좌석 해제(환불 없음). **왜**: "신청 따로/결제 따로"는 카드사에 흐름을 설명해야 하는 비표준 → "주문 즉시 결제"가 표준이라 심사 리스크↓ + 어차피 붙일 방향. (2회차+ 는 아직 강사 사전수락 후 결제 — 스코프 1회차.)
...
- **결제 전**(미결제 `PENDING`, 2회차 `PAYMENT_PENDING`) = 순수 취소(무료). 강사가 풀을 안 잡아 손해 0.
...
- 회차 상태(`EnrollmentStatus`, 6값): (선결제 1회차) `PENDING`(미결제·결제 대기) → `ACCEPT_PENDING`(결제완료·강사 확인 대기) → `CONFIRMED`(강사 수락·확정) / `REJECTED`(1회차 거절·복구 가능·자동환불) / `CANCELLED`(취소·만료, 결제완료분 자동환불). (2회차+) `PAYMENT_PENDING`(강사 사전수락·결제 대기) → `CONFIRMED`. **done = `CONFIRMED && doneAt!=null`**(별도 enum 없음). 강의(수강) 상태는 회차들에서 파생.
- **취소 무료 = 결제 전**(미결제 PENDING, 2회차 PAYMENT_PENDING). 거절/취소로 session 활성 신청이 0 이 되면 session 은 잔존(bind/unbind 없음), 이력 스냅샷은 보존.
...
| 2026-08-07 | **선결제 전환(1회차)**: 신청 즉시 결제 → 강사 수락/거절(자동환불), 신규 상태 `ACCEPT_PENDING` | "신청 따로/결제 따로"는 카드사에 흐름 설명해야 하는 비표준 → "주문 즉시 결제" 표준으로 심사 리스크↓ + 어차피 붙일 방향. 거절→자동환불은 검증된 PG cancel 재사용(이벤트→listener). 오버부킹=이중결제라 좌석 하드닝(비관락+세션 UNIQUE V12). 스코프=1회차(2회차+ 구 흐름 유지). |
