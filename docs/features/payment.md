...
- **🔴 NAT 게이트웨이 + EIP 로 아웃바운드 IP 고정 후 파트너관리자에 등록** — KCP 는 **취소를 등록된 서버 IP 에서만** 허용한다(에러코드 8012). 현재 ECS 는 `assign_public_ip=true`(NAT 없음)라 IP 가 매번 바뀌어 **환불이 배포마다 깨진다**. 경로: 파트너관리자 → 기술관리센터 → 보안관리 → 서버 IP 설정(결제). (승인은 인증서 인증이라 IP 무관 — 취소만 해당.)
