...
| 2026-08-10 | **더 비싼 슬롯 = 차액만 추가 결제**(V16) — 목표 슬롯을 주문에 싣고 승인 순간 교체, 결제창 동안 좌석 hold, 미결제면 주문만 만료 | 400 으로 막고 "취소→재신청"으로 우회시키면 **전액 환불 후 전액 재결제**라 30만원대 강의에서 돈이 며칠 묶인다. 대기를 예약이 아니라 **주문**에 두면 방금 없앤 `PAYMENT_PENDING` 류를 되살리지 않고도 결제 대기를 표현할 수 있다 — 회차 상태는 내내 그대로고, 포기하면 주문만 만료돼 롤백할 것이 없다. 강사 제안도 더 비싼 슬롯을 낼 수 있게 필터 완화 |
