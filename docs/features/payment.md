...
| 2026-08-15 | **그레이스 앵커 = 강사 수락(확정) 시각으로 확정**(#258 ② 되돌림; ①③ 유지) | staging 실사고: 결제 14:04 → 강사 수락 19:02 → 19:04 환불 신청(당일 세션)인데 결제 시각 앵커라 그레이스 만료 → 1회차 (수강료/3+부대) 0% = 193,666원 미환불. 사용자 최종 정책은 **수락 후 1h 내 100%** — 페널티 명분(수락 후 풀 예약→부대비용 손실)이 생기는 시점이 수락이라 시계도 거기서. #258 ②는 대화에서 이미 엎었던 방향이 코드에 남은 것. 확정 후 respondedAt 갱신 경로가 없어 재개방 우려는 실재하지 않음(RefundCalculatorTest F3/N2 재작성, RefundUseCaseTest RF16 재현) |
...
- **환불율**(CONFIRMED 한정): 당일 0 / 전날 50 / 2일전 70 / 3일전+ 100, 단 **강사 수락(확정) 1h 내 100**(그레이스). 그레이스 앵커는 **확정 시각**(CONFIRMED 회차의 `respondedAt` = 수락·pick-slot·재수락 순간). 왜 수락인가: 페널티의 명분이 "강사가 수락하고 풀을 예약한 뒤의 취소 = 부대비용 손실"이므로 시계도 그 시점부터 돈다. 결제 시각 기준(#258, 2026-08-14~15 잠깐)은 강사가 늦게 수락할수록 학생이 확정 직후 취소해도 페널티를 맞는 **소비자 기만**이었다 — "강사가 새벽에 수락하면 학생이 불리하다"는 우려는 반대다(늦은 수락 = 미수락 100% 구간이 길어질 뿐이고, 확정 뒤 취소할 사람은 그 1시간에 하면 된다). 확정 후 `respondedAt`을 갱신하는 경로는 없어(제안·재요청은 ACCEPT_PENDING으로 되돌아간 뒤에만 가능, 그 상태는 100%) 그레이스가 조용히 재개방되지 않는다. (코드 상수 — SiteSettings 이전은 튜닝 필요 시.)
