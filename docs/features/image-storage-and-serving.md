...
- 비공개(자격증·보험)는 별도 스토리지 인터페이스를 타고 공개 서빙 경로가 아니라 이번 범위에서 제외. (레거시 lecture 이미지도 제외 대상이었으나 2026-08-15 삭제되어 더 이상 존재하지 않는다.)
...
- ~~🟡 **레거시 `/lecture`·`/lectureImage` 이미지**~~ — ✅ 해소(2026-08-15). 엔드포인트·테이블이 삭제됐다. ⚠️ 단, S3 에 남은 옛 lecture 이미지 **객체**는 이 삭제로 지워지지 않는다(고아 객체 — 정리 여부는 백로그).
