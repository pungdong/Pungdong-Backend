...
`enrollmentId` 는 **소유 + 완료 + 종목 정합** 3중 검증을 거친다. 완료 판정은 **`enrollment/EnrollmentCompletion`** 을 쓴다 — 강의일정 hub 가 카드 상태를 `COMPLETED` 로 내릴 때와 **같은 식**이다.

> 🔴 **이 공유를 깨지 말 것.** 판정이 두 벌이 되면 **FE 피커에 뜬 강의를 BE 가 400 으로 거절**한다. `CourseScheduleStatus.derive()` 만 쓰면 안 된다 — 그건 "잡힌 회차"만 봐서 3회차 중 1회차만 듣고 끝낸 수강도 완료로 판정한다. 코스의 **정규 회차 수**와 대조해야 한다.
