 ...
 *
 * <p><b>필드는 {@link StudentCertificateCreateRequest} 와 같아야 한다</b> — 한쪽에만 필드를 추가하면
 * 등록은 받는데 수정은 <b>조용히 무시</b>하는 어긋남이 생긴다. 그럼에도 클래스를 나눈 이유는
 * <b>같은 필드가 다른 뜻을 갖기 때문</b>이다(아래 두 항목). 이름이 같다고 의미까지 같지 않다.
 *
 * <ul>
 *   <li>{@code photoFileKey} — 등록에선 <b>필수</b>({@code @NotBlank}), 수정에선 <b>"없음 = 기존
 *       사진 유지"</b>. FE 편집 폼이 기존 사진을 다시 업로드하지 않게 하려는 것이다. 그래서 여기엔
 *       <b>{@code @NotBlank} 를 걸 수 없다</b> — 빈 값이 정상 입력이다. 대신 "결과가 사진 없는
 *       자격증이면 안 된다"를 서비스가 검사한다({@code requirePhotoAfterUpdate}) — 사진 없이 등록된
 *       옛 행은 수정하는 김에 사진을 붙여야 한다. (사진 <i>제거</i>는 이 계약으로 표현할 수 없다 —
 *       화면에도 제거 버튼이 없다. 생기면 별도 필드가 필요하다.)</li>
...
public class StudentCertificateUpdateRequest {
     ...
     *
     * <p>단 <b>기존 사진도 없으면 400</b> 이다("자격증 사진을 추가해주세요."). 사진은 필수인데 이 필드에
     * {@code @NotBlank} 를 걸면 "유지" 의미론이 죽어서, 검사를 서비스로 옮긴 것이다.
     */
    @Size(max = 500)
