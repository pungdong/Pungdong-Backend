...
- **발송 쿨다운 (2026-07-10)** — create·resend 에 계정당 발송 간 최소 간격(`send-cooldown-seconds`, 기본 30s, Redis `SET NX EX` 키 `identity:otp:cooldown:<accountId>`) 강제. 다날 **실 SMS 비용·남용(toll-fraud)** 방어 — FE 만 믿지 않는다(만료 시 재전송이 화면 중앙 큰 버튼이라 연타 쉬움). 초과 시 예외 아니라 **200 + `retryAfterSeconds`**(레포 규약: 클라 분기하는 정상 결과 = 200+필드, "N초 후 재시도"는 정상 UI). SMS 미발송·레코드 미생성(create)/상태 불변(resend). **응답은 discriminated union** — 쿨다운은 **순수 `{retryAfterSeconds}`**(성공 필드 없음, resend 도 verificationId·링크 안 실음), 성공은 `{status:READY, verificationId, otpExpiresInSeconds, otpExpiresAt}`. **`IdentityVerificationResult` 에 `@JsonInclude(NON_NULL)`** 로 각 형태에 없는 필드를 JSON 에서 빼야 union 이 진실(안 그러면 쿨다운에 성공 필드가 null 로 실려 FE 타입이 거짓말 → NaN/0 타이머). FE types.ts 도 union(`IdentityVerificationSent | IdentityVerificationCooldown`). **config-gated** — test 프로파일 0(비활성)이라 기존 테스트 무영향, 전용 `IdentityVerificationRateLimitUseCaseTest`(RL1/RL2, embedded Redis)가 검증. ★**시간당 상한(cap)은 real 모드 전환(다날 CPID 개통) 전 후속** — 현재 prod=stub 이라 실 SMS 미발송이라 아직 여유.
