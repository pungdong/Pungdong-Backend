...
- **OTP 만료 = 상대초로 내림 (2026-07-10)** — 발송/재발송 응답의 `otpExpiresInSeconds`(서버 계산 잔여 초)가 FE 카운트다운의 **단일 출처**. `otpExpiresAt`(LocalDateTime, 오프셋 없음)은 표시/디버그용. **왜**: 컨테이너 JVM=UTC 라 offset 없는 `otpExpiresAt` 을 FE(KST)가 9h 과거로 읽어 즉시 만료로 표시되던 prod 버그 → 근본봉합으로 `PungdongApplication.main` 에서 전역 `TimeZone.setDefault(Asia/Seoul)` 고정(모든 LocalDateTime 필드 정합) + 카운트다운은 시계/TZ 무관한 상대초로. TTL 은 stub 180s/real 300s(하드코딩 금지). 교훈 = memory `feedback_container_tz_localdatetime`. ★**`otpExpiresInSeconds` 는 영구 필드 — UTC+오프셋 글로벌화 이후에도 유지(제거 금지).** 그때 FE 는 절대시각을 primary 앵커로 승격하되 이 잔여초를 **sanity check/fallback** 으로 남긴다(기기 시계가 서버와 크게 어긋나 아직 유효한 코드를 조기 잠그는 = 서버가 구제 못 하는 실패 방어). 글로벌화 작업 시 이 필드 계속 내릴 것.
- **발송 쿨다운 (2026-07-10)** — create·resend 에 계정당 발송 간 최소 간격(`send-cooldown-seconds`, 기본 30s, Redis `SET NX EX` 키 `identity:otp:cooldown:<accountId>`) 강제. 다날 **실 SMS 비용·남용(toll-fraud)** 방어 — FE 만 믿지 않는다(만료 시 재전송이 화면 중앙 큰 버튼이라 연타 쉬움). 초과 시 예외 아니라 **200 + `retryAfterSeconds`**(레포 규약: 클라 분기하는 정상 결과 = 200+필드, "N초 후 재시도"는 정상 UI). SMS 미발송·레코드 미생성(create)/상태 불변(resend). **응답은 discriminated union** — 쿨다운은 **순수 `{retryAfterSeconds}`**(성공 필드 없음, resend 도 verificationId·링크 안 실음), 성공은 `{status:READY, verificationId, otpExpiresInSeconds, otpExpiresAt, resendAvailableInSeconds}`. **`resendAvailableInSeconds`**(성공에만) = 다음 발송까지 남은 초(방금 잡은 쿨다운 창=config, 0=없음) → FE 가 발송 직후 재전송 버튼 pre-disable+카운트다운("눌러봐야 아는 버튼" 방지). 차단 시 `retryAfterSeconds` 와 같은 개념(쿨다운 잔여)을 성공 상태에서 표현. **`IdentityVerificationResult` 에 `@JsonInclude(NON_NULL)`** 로 각 형태에 없는 필드를 JSON 에서 빼야 union 이 진실(안 그러면 쿨다운에 성공 필드가 null 로 실려 FE 타입이 거짓말 → NaN/0 타이머). FE types.ts 도 union(`IdentityVerificationSent | IdentityVerificationCooldown`). **config-gated** — test 프로파일 0(비활성)이라 기존 테스트 무영향, 전용 `IdentityVerificationRateLimitUseCaseTest`(RL1/RL2, embedded Redis)가 검증. ★**시간당 상한(cap)은 real 모드 전환(다날 CPID 개통) 전 후속** — 현재 prod=stub 이라 실 SMS 미발송이라 아직 여유.

- **요청 형식 검증 + KR 결합 목록화 (2026-07-10)** — 휴대폰·생년월일·OTP 에 형식 검증 도입. 그 전엔 `@NotBlank` 뿐이라 **형식조차 안 보고 유료 외부 기관(다날)을 호출**했다(`birth` 는 `toIsoDate` 가 비-8자리를 그대로 흘려보냄, `otp` 는 아무 문자열이나 5회 시도 한도를 깎음). FE 검증은 UX 용이지 방어선이 아니다(콘솔·직접 호출로 우회).
  - **검증은 DTO(`@Valid`)에** — 컨트롤러 진입에서 돌아 `acquireSendSlot()`(30s 쿨다운)보다 **앞선다**. 오타가 사용자를 30초 잠그지 않는다(`RL3` 가 못박음). 서비스에 뒀다면 태웠다.
  - **정규화 후 검증** — `phoneNumber`/`birth` setter 가 구분자를 뗀다. 기존 use-case 테스트가 `010-1234-5678`(하이픈)을 보내고 있었고 그게 `real` 이었다면 하이픈째로 다날에 나갔다. 저장·전송 모두 canonical(숫자만).
  - **`otp` 6자리 숫자 위반 = 400 이고 `attemptCount` 미소모** — 시도 횟수는 진짜 추측만 센다. ⚠️ FE 에 보이는 유일한 동작 변경(전엔 200 FAILED). FE 는 6자리 전 제출을 막을 것.
  - ★ **KR 규칙을 그대로 박되 국가 추상화는 짓지 않는다.** 이 도메인은 이미 통째로 한국 종속(`Carrier`·CI/DI·`ForeignerType`·다날) — regex 는 그 중 가장 약한 결합이다. 일본 확장은 regex 파라미터화가 아니라 `IdentityVerifier` 뒤의 새 구현 + 다른 레코드 모양이고, **그 확장점은 이미 있다**. 근거: 저장된 모든 row 가 KR 이라 E.164 전환이 `UPDATE … CONCAT('+82', …)` 한 줄 = **되돌릴 수 있는 결합**. (반대로 naive `LocalDateTime` 은 되돌릴 수 없이 모호해 지금 UTC 로 옮긴다 — `docs/architecture/time-handling.md` §2.) 추상화 대신 **결합 지점을 목록으로** 고정: 피처 문서의 "한국 종속 인벤토리". **가입 시 `country` 필드도 안 만든다**(국적/거주국/마켓/번호국가코드 4개가 뭉개짐 + 신원 속성은 가입 아닌 인증 게이트에서 수집 — `SignUpUseCaseTest` S3).

## 안전망 테스트

`src/test/.../usecase/IdentityVerificationUseCaseTest`(S1 create·S2 confirm·S3 재사용·**S4 번호 정규화**·V1 오답·**V2~V4 형식 거부(레코드 미생성)**·**V5 OTP 형식(시도 미소모)**·D1 resend·T1/T2 미인증·R1 비소유). 쿨다운은 `IdentityVerificationRateLimitUseCaseTest`(RL1/RL2·**RL3 형식오류는 창 미획득**). skip + status 게이트는 `InstructorApplicationUseCaseTest` 가 create→confirm→submit 으로 검증. 실 PortOne 클라이언트는 라이브 미검증(CPID 개통 후 수동).
