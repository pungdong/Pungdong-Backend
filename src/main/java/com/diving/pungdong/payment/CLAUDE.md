...
- **환불 정책 앵커**(#258) — 그레이스(결제 1h 내 100%) 기준 = **결제완료 `approvedAt` 회차 최소(불변)**. `respondedAt`(가변)은 강사 수락·일정변경마다 리셋돼 "당일인데 100%" 버그 — 금지. 날짜 페널티는 **CONFIRMED(풀 예약)에만**, 미수락·미결제는 100%(cancel 경로와 일치). 수강료/N 나머지는 마지막 정규회차 배분(환불 합계=원금).
