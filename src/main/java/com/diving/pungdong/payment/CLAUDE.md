...
- **서비스**: `PaymentService`(권위 금액·멱등 prepare·PG 승인·회차 확정. ⚠️ **빈 이름 `@Service("enrollmentPaymentService")`** — 레거시 단순명 충돌 회피). **`RefundService`**(수강 종료 — `RefundCalculator` 산정 + 주문별 PG 부분취소 + 회차 CANCELLED + 좌석 해제). **`RefundCalculator`**(회차별 환불 정책: done=0·미배정=수강료/N(100%)·배정취소=(수강료/N+부대)×율; **날짜 페널티율(당일0/전날50/2일전70/3일전+100)은 CONFIRMED 에만** — 미수락(ACCEPT_PENDING)·미결제는 100%(#258); 그레이스(**강사 수락 1h 내** 100%) 앵커 = **CONFIRMED 회차의 `respondedAt`(확정 시각)** — 결제 시각 아님(#258 ② 되돌림, 2026-08-15); 나눗셈 나머지는 마지막 정규회차(합계=원금); **수강료 몫은 1회차 주문**, 부대는 각 회차 주문).
- **컨트롤러**: `PaymentController` — `POST /payments/prepare`·`confirm`(TOSS/STUB, 학생 인증)·**`GET /payments/orders/{orderId}`**(상세조회, 소유권). **`InicisReturnController`** — **`POST /payments/inicis/return`(permitAll)** = 이니시스 결제창 form POST 콜백 → 세션리스 승인 → 302 리다이렉트(주문 `client` 로 web/app). ⚠️ 앱 WebView 가 POST 본문을 못 읽어 이니시스는 confirm 주체가 BE. **`RefundController`** — `POST /enrollments/{enrollmentId}/refund`(수강 종료=남은 회차 환불; enrollment 경로지만 PG 취소라 payment 패키지 — enrollment→payment 역참조 방지). **`AdminRefundController`** — `POST /admin/payments/orders/{orderId}/refund`(hasRole ADMIN, **운영자 수동 환불**: 정책 오산정·CS 보정으로 남은 잔액/일부를 돌려줄 때 **PG 콘솔 대신 이걸로** — `RefundService.refundOrderManually` → 같은 `applyCancel` 이라 `RefundOrder` 원장·`refundedAmount`·PG 라우팅·이중환불 가드가 그대로. 돈만 만지고 회차/수강 상태는 불변. `amount` 생략=잔액 전액, 초과는 400(clamp 안 함). 첫 사용: 2026-08-15 staging 그레이스 오산정 잔액 193,666 보정).
- **서비스**: `PaymentService`(권위 금액·멱등 prepare·PG 승인·회차 확정. ⚠️ **빈 이름 `@Service("enrollmentPaymentService")`** — 레거시 단순명 충돌 회피). **`RefundService`**(수강 종료 — `RefundCalculator` 산정 + 주문별 PG 부분취소 + 회차 CANCELLED + 좌석 해제). **`RefundCalculator`**(회차별 환불 정책: done=0·미배정=수강료/N(100%)·배정취소=(수강료/N+부대)×율; **날짜 페널티율(당일0/전날50/2일전70/3일전+100)은 CONFIRMED 에만** — 미수락(ACCEPT_PENDING)·미결제는 100%(#258); 그레이스(결제 1h 내 100%) 앵커 = **결제완료 `approvedAt` 최소(불변)** — respondedAt 은 수락·일정변경마다 리셋돼 금지; 나눗셈 나머지는 마지막 정규회차(합계=원금); **수강료 몫은 1회차 주문**, 부대는 각 회차 주문).
- **원장·대사(2026-08 하드닝, #249·#252·#253·#261)**: **`PaymentApprovalLedger`**/`PaymentApproval`(`payment_approval`, V22) — **승인 시도 원장**(`REQUIRES_NEW` 별도 빈, `RefundLedger` 와 대칭): PG 호출 직전 `ATTEMPTED` 선기록 → `APPROVED`(+tid)/`FAILED`. orphan charge("카드는 청구됐는데 DB 흔적 0") 차단. **`PaymentCallbackRecorder`**/`PaymentCallbackLog`(V24)/`CallbackOutcome` — 이니시스 콜백 수신을 **성패·위조(미상 P_OID)·인증실패 포함 전부** REQUIRES_NEW 로 기록(기록 실패는 삼킴 — 결제 경로를 막지 않음), 승인 실패는 예외 스택까지 로깅. **`PaymentReconciliation`** + `PaymentReconciliationScheduler`(`@Profile("!test")`, `pungdong.payment.reconciliation-sweep-ms` 기본 10분) — ① `reportStuck`: 15분+ 결과 미확인 승인(ATTEMPTED)·환불(REQUESTED)을 ERROR 로 ② `reportAmountMismatch`(M1): ACCEPT_PENDING·CONFIRMED 회차의 순액≠`chargeTotal()` 드리프트를 ERROR 로(미결제/취소 제외·respondedAt<cutoff·REQUESTED in-flight 회차 제외로 오탐 방지). **탐지만** — 자동 재승인/재환불 X.
...
- **`PaymentGatewayRegistry`** — 어느 PG 로 보낼지 고른다. 어댑터는 `@ConditionalOnProperty` 없이 **전부 빈**으로 뜨고, 레지스트리가 선택: `active()`=전역 설정(신규 결제) / `forOrder(order.provider)`=주문에 박제된 PG(기존 주문 승인·환불). **왜**: PG 를 갈아탄 뒤 과거 주문 환불이 엉뚱한 PG 로 나가면 "돈은 받고 환불 실패"(FE 리뷰). `PaymentOrder.provider` 는 prepare 가 박고(V10), 승인·환불은 그 값으로 라우팅.
  - **이니시스 특이점**(INIpay PRO, P_ 스킴): `initParams` 는 **외부 호출 없이** P_ 파라미터+서명 `P_CHKFAKE`(=Base64(SHA-512(P_AMT+P_OID+P_TIMESTAMP+hashKey)))만 계산 — FE 가 `INIPayPro_v2.js` 로 결제창을 띄운다. **승인(payAppl.ini)엔 서명이 없다** → 금액은 콜백값이 아니라 주문 권위값으로 대조. **승인 호스트를 콜백 `P_IDCNAME`으로 조립**하므로 `idcHost()`가 소문자 토큰만 허용(SSRF 방어). **환불(iniapi V2 JSON)** 의 `hashData`(SHA-512hex)는 body 의 `data`와 **바이트 동일**해야 통과 → `data`를 한 번만 직렬화해 양쪽에 쓴다. 부분취소 `confirmPrice`=취소 **후** 잔액(포트 `remainingAmount`−`cancelAmount`). ★**부분취소 이력이 있는 원거래는 잔액 전액도 전체취소(refund)가 거절된다(500624 "부분취소 원거래 취소불가")** — 그래서 포트 `cancel(tid, 취소액, 잔액, **원금**, 사유)` 에 원금을 싣고, 어댑터는 `잔액==원금(이력 없음) && 전액` 일 때만 `refund`, 그 외 전부 `partialRefund(confirmPrice=취소 후 잔액, 0 가능)`. 우리 원장이 아는 사실을 넘겨 처음부터 맞는 전문을 고른다(PG 거절 코드로 되짚지 않음). 2026-08-15 staging 수동환불에서 발견. 테스트/운영은 엔드포인트가 아니라 **MID**(테스트 `INIpayTest`)로 갈려 live 플래그 없음.
- **엔티티**: `PaymentOrder`(orderId·**enrollmentRound**·amount(권위 금액)·status·paymentKey·**`@Version`**(V20)…) → `PaymentOrderJpaRepo`. **`RefundOrder`**(paymentOrder·amount·reason·status — 주문별 환불 감사기록) → `RefundOrderJpaRepo`. **`PaymentApproval`**(승인 시도 원장) → `PaymentApprovalJpaRepo`. **`PaymentCallbackLog`**(콜백 수신) → `PaymentCallbackLogJpaRepo`. enum `PaymentStatus`(READY/DONE/CANCELED/FAILED), `RefundStatus`(REQUESTED/DONE/FAILED), `ApprovalStatus`(ATTEMPTED/APPROVED/FAILED), `CallbackOutcome`(UNKNOWN_ORDER/AUTH_FAILED/APPROVED/APPROVAL_FAILED).
- **dto/**: `PaymentPrepare/Confirm Request/Response`(+**`orderNo`** = CS·고객용 주문번호), **`RefundQuote`**(total + 회차별 line: tuitionPart/extraPart 분리 — 실행 매핑용).
  - **`PaymentConfirmResponse` 는 두 축이 섞인 DTO**(2026-08-11 개명) — **결제의 결과**(`status`·`amount`·**`scheduleChange`**)는 멱등, **`currentEnrollmentStatus`** 는 회차를 **live 로** 읽어 멱등이 아니다(강사가 그새 수락하면 `CONFIRMED` 가 온다). 옛 이름 `enrollmentStatus` 가 "결제의 결과" 로 읽혀 FE 회귀를 만들어 개명했다. `enrollmentId` → **`roundId`**(담는 값이 회차 id 인데 환불 경로의 수강 id 와 혼동). **`refundedAmount`**(누적 환불액)·**`refundableAmount`**(=amount−refundedAmount) 도 싣는다(#262, M5) — `status` 만으론 부분환불 금액이 안 보여 FE 가 "N원 환불됨"·잔액을 계산 없이 표시.
...
- **기록은 `RefundLedger` 가 별도 트랜잭션(`REQUIRES_NEW`)으로** — 환불은 롤백 안 되는 외부 부수효과라, 기록을 발행자 트랜잭션에 묶으면 롤백 시 "PG 엔 나갔는데 DB 엔 없는" 상태가 된다. **PG 호출 직전 `REQUESTED` 선기록 → `DONE`(+잔액 누적) / `FAILED`(+PG code·msg)**. `REQUESTED` 잔존 = **결과 미확인** → `RefundBlockedException`(**409/-1022**)으로 **발행자(거절·취소·만료)까지 롤백**(C2 — "환불 못 하면 상태 전이도 확정 안 함"; 사람이 PG 원장 대사 후 확정하면 재시도가 흐른다). PG 가 2xx 인데 `canceled=false` 면 `DONE` 기록 안 하고 `FAILED`+rethrow(H-2). ⚠️ 별도 빈인 이유 = self-invocation 이면 프록시를 안 거쳐 `REQUIRES_NEW` 가 무시된다.
- **승인도 시도부터 원장** — `PaymentApprovalLedger`(`REQUIRES_NEW` 별도 빈, C1): PG 호출 직전 `ATTEMPTED` 선기록 → `APPROVED`(+tid) / `FAILED`. 확정(`finalizeApproval` — 주문 DONE·회차 전이)이 롤백돼도 **청구 사실은 durable** → 재시도가 `findApproved` 로 **재청구 없이 전진 확정**(정확히 한 번 청구/여러 번 적용). `ATTEMPTED` 잔존 = 결과 미확인 → `hasUnresolvedApproval` 이 재승인 차단(이중청구 방지, 대사 대상). 조회 가드도 `REQUIRES_NEW` — 발행자 스냅샷에 갇히면 남이 방금 커밋한 시도를 못 본다. 회계 불변식: **DONE 인데 `approvedAt=null` 금지** — PG 가 승인시각을 안 주면 처리시각 보정+warn.
- **콜백 수신은 전부 기록** — `payment_callback_log`(V24)에 위조(UNKNOWN_ORDER)·인증실패(AUTH_FAILED)·승인성공(APPROVED)·승인실패(APPROVAL_FAILED)를 REQUIRES_NEW 로 무조건 남긴다. "받았는데 기록 0" 분쟁·공격 탐지·실패 콜백의 `P_AUTH_TID`(이니시스에 되물을 유일한 키) 보존.
- **동시성의 최종 심판은 DB 제약** — 가드는 락 없는 pre-check 라 near-simultaneous 를 못 막는다. ① `@Version` 낙관락(`EnrollmentRound`·`PaymentOrder`, V20) — blind full-row overwrite 방지, 충돌 = **409/-1021** ② `payment_order` READY 조건부 유니크(V21, 가상 생성컬럼) — 동시 prepare 이중 READY(이중청구·영구 500) 차단, 진 쪽 `ConcurrentRequestException`(409/-1021) ③ `refund_order` in-flight 유니크(`uk_refund_order_inflight`, V26) — 동시 이중환불 원자 차단. **FOR UPDATE 는 못 쓴다** — `markDone` 이 `REQUIRES_NEW`(다른 커넥션)로 같은 주문 행을 UPDATE 하려다 바깥 FOR UPDATE 와 self-deadlock ④ 좌석은 `EnrollmentService.requireSeat` 가 세션 `lockById`(FOR UPDATE) + **잠금 count**(`lockOccupyingRoundIds`) — plain count 는 MySQL RR 스냅샷에 갇혀 overbooking(H-4). ⚠️ 이 의미들은 H2 가 재현 못 한다 — **실 MySQL 하네스 `./gradlew mysqlTest`**(Testcontainers, `@Tag("mysql")`)가 지킨다.
- **대사는 탐지만** — 10분 스윕(`PaymentReconciliationScheduler`)이 15분+ 미확정 시도와 순액≠`chargeTotal()` 드리프트(M1)를 ERROR 로 표면화. 자동 재승인/재환불은 안 한다(이중청구·이중환불 위험) — 확정은 사람이.
- **환불 정책 앵커**(2026-08-15 확정) — 그레이스(**강사 수락 1h 내** 100%) 기준 = **CONFIRMED 회차의 `respondedAt`(확정 시각)**. 페널티 명분(수락 후 풀 예약→부대비용 손실)이 생기는 시점이 수락이라 시계도 거기서. 결제 시각(`approvedAt`) 앵커(#258)는 늦은 수락 뒤 즉시 취소에도 페널티를 물려 되돌림. 확정 후 `respondedAt` 갱신 경로 없음(제안·재요청은 ACCEPT_PENDING 에서만) → 재개방 없음. **CONFIRMED 회차의 respondedAt 을 건드리는 코드를 추가하면 그레이스가 리셋된다 — 하지 말 것.** 날짜 페널티는 **CONFIRMED(풀 예약)에만**, 미수락·미결제는 100%(cancel 경로와 일치). 수강료/N 나머지는 마지막 정규회차 배분(환불 합계=원금).
...
**하드닝 에러코드**: **`-1021 CONCURRENT_MODIFICATION`**(409 — `@Version` 낙관락 충돌·동시 prepare 유니크 충돌, "잠시 후 재시도") / **`-1022 REFUND_BLOCKED`**(409 — 환불 대사 가드·동시 환불 충돌로 상태 전이째 롤백). `ExceptionAdvice`·`exception_ko.yml`·`types.ts` ErrorCode 동기.

## 설정

`pungdong.payment.mode`(**stub|toss|inicis**, 부팅 시 하나만) + `toss.secret-key`/`client-key` + `inicis.mid`/`hash-key`(P_CHKFAKE 서명)/`api-key`(환불 hashData)/`client-ip`(환불 전문 IP)/`ret-url`(=**BE 콜백 URL** `.../payments/inicis/return`)/`return-web-success|fail`(환경별)/`return-app-success|fail`(기본 `plop://payment/...`) — `application.yml`·`.env.example`. 로컬 stub 기본(외부 미호출). 이니시스 테스트/운영은 엔드포인트가 아니라 **MID**(테스트 `INIpayTest`)로 갈려 live 플래그 없음. 키 발급 전 토스 **문서용 테스트 키**도 사용 가능(`.env.example` 주석). 대사 스윕 주기는 `pungdong.payment.reconciliation-sweep-ms`(기본 600000 = 10분, `@Scheduled` fixedDelay — 스케줄러는 `@Profile("!test")`).
...
`src/test/.../payment/InicisPaymentTransmissionTest` — 이니시스 전문 사양(외부 호출 0): 승인 전문이 서버 권위 금액을 싣는지(K1), P_CHKFAKE 서명 공식(K2), 환불 hashData `data` 바이트동일성(K3/K4), 부분/전체취소 분기(K5), SSRF idcHost 방어(V2). `PaymentUseCaseTest`: P1(prepare)·P2(confirm→확정)·P3(금액불일치)·P4(멱등)·P5(결제대기 아님)·P6(비소유)·P7(점유→둘째 수락 차단) + **A2~A3**(승인 원장 APPROVED durable·재청구 없는 전진 확정) + **W1~W3**(결제 잔여 초 노출·결제 후 소멸·일반결제 `scheduleChange=false`) + **I1~I7 이니시스 콜백**(승인·거절·인증실패·위조 + I5 낯선 Origin 도 CORS 통과 + I6 전역 CORS 유지 + I7 콜백 수신 DB 기록·authTid 보존) + O1~O2. **차액 결제 사양은 `MultiRoundProgressUseCaseTest` 의 C1~C5·PH5** (C1 reschedule -1018 / C1-1 pick-slot -1018 / C1-2 정원1 데드엔드 / C1-3 정원1 reschedule / C2 시간포맷 / C3 scheduleChange / C4 -1019 / C5 위치 가드 / PH5 -1020). ⚠️ Authorization raw JWT.
하드닝 안전망: `payment/PaymentReconciliationTest`(RC1~RC4 — 유예·순액 정합·드리프트·in-flight 제외), `payment/PaymentOrderConcurrencyTest`(VL1~VL2 낙관락), `payment/RefundCalculatorTest`(F1~F6·N1~N3 — 그레이스 앵커=수락 시각·CONFIRMED 한정 페널티·나머지 배분), `usecase/RefundUseCaseTest`(RF7~RF16·RF17~RF19 어드민 수동 환불 — clamp·CANCELED·FAILED 이력·`-1022` 가드·C2 롤백·H-2·다주문). **동시성 실증은 `./gradlew mysqlTest`**(Testcontainers 실 MySQL, `concurrency/` — H-1 이중환불·H-4/H-4b overbooking·하네스 스모크. H2 는 FOR UPDATE·RR 스냅샷 의미를 재현 못 해 기본 스위트에서 제외, Docker 없으면 skip).
