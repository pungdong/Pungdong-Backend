...
- **첫-동의 freeze 트리거** — Sanity publish 웹훅 대신 "처음 동의 시 BE 가 fetch+freeze". 웹훅 셋업 불필요(MVP). 동시 첫-동의 경합은 UNIQUE 제약이 막고 500 → FE 재시도로 성공(허용).
...
`src/test/.../usecase/ConsentUseCaseTest` — 실제 H2 + 시큐리티, `SanityTermClient` 만 `@MockBean`. `C1` 첫 동의 박제 / `C2` 같은 버전 재사용(freeze 1회) / `C3` 새 버전 별도 박제 / `V1` 없는 약관 400 / `V2` 빈 동의 400 / `C4` GET /me 이력.
