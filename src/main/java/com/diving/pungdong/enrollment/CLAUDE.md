...
- **제안 hold 회수는 "제안을 비우는 모든 경로"에서(2026-08-11)** — `proposedSlots.clear()` 하는 곳은 hold 도 같이 풀어야 한다. `pickSlot` 은 원래 했지만 `swapSlot`(reschedule·supersede)·`applySlotChange`(차액 승인)는 안 해서, 학생이 제안을 안 고르고 다른 슬롯으로 가면 **아무도 못 쓰는 고아 hold** 가 남아 `proposalTtlHours`(6h) 동안 남의 신청을 막았다(정원 1이면 그 시간대가 통째로 잠김). 스윕이 결국 풀어주지만 6h 는 길다. 고정 = `M6-1`·`C1-3`.
