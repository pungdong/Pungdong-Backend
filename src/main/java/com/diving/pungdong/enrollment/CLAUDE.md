...
- **엔티티 (다회차 2026-06-28)**: **`Enrollment`(수강 컨테이너 — student·course·**tuitionSnapshot**·createdAt·`rounds[]`)** ⊃ **`EnrollmentRound`(회차 — courseRound FK·roundIndex·roundKind·**availabilitySession**·venueRefId·date·block·ticketRef·status·**entry/equip/extra 스냅샷**·doneAt·rejectionReason)** ⊃ **`EnrollmentRoundEquipment`(itemRef·name·price·**size**)**. 수강료는 수강에 1번(1회차 결제에 전액), 부대비용은 회차별. 강의 상태는 회차들에서 파생(`RoundScheduleStatus`/`CourseScheduleStatus`). API 의 `{id}` = **회차 id**. payment 응답은 `roundId`(2026-08-11 개명 — 옛 `enrollmentId`); payment **요청**의 `enrollmentId` 는 하위호환으로만 병행. enum `EnrollmentStatus`(**5값** — PENDING(미결제)/**ACCEPT_PENDING**(결제완료·강사 결정 대기)/CONFIRMED/REJECTED/CANCELLED — `isActive()`/`occupiesCapacity()` + `ACTIVE`/`OCCUPYING` 집합 상수, done=CONFIRMED+doneAt). 슬롯·상태·점유 집계는 `EnrollmentRoundJpaRepo`.
...
- **결제 카운트다운 = 파생, 저장 안 함(2026-08-11)** — 미결제 회차의 잔여 초는 `PaymentWindow`(순수 함수)가 `createdAt + paymentTtlHours` 로 읽을 때 푼다. 만료 스윕의 컷오프(`now - ttl`)와 **같은 식의 뒤집음**이라 둘을 같이 고쳐야 화면과 실제 만료가 안 어긋난다. **절대시각이 아니라 잔여 초**로 내리는 이유는 기기 시계 오차([[feedback-container-tz-localdatetime]]). ⚠️ 0 ≠ 결제 불가 — 스윕이 5분 폴링이라 잠깐은 결제가 성사된다(성사되면 `ACCEPT_PENDING` 이 되어 스윕 대상에서 빠지므로 "결제했는데 취소" 구멍은 없다).
- **더 비싼 슬롯 거부 = 전용 코드 -1018(2026-08-11)** — `settleSlotChange` 의 "금액 늘어남" 거부가 `BadRequestException`(-1011)이라, reschedule 의 **다른 9가지 실패**(만석·확정 회차·슬롯 무효·coverage 밖 …)와 구분이 안 돼 FE 가 사유를 못 가렸다. `AdditionalPaymentRequiredException`(-1018, 400 유지)으로 분리. **`pickSlot` 에서도 나온다** — 강사가 더 비싼 슬롯을 제안하는 건 허용이고(2026-08-10), 학생이 그걸 고르면 차액 결제로 가야 하기 때문. FE 는 이 코드일 때만 `POST /payments/prepare` + `target*` 로 유도한다.
- **제안발 차액 결제도 강사 재수락(2026-08-11 확정)** — 더 비싼 제안을 학생이 고르면 `-1018` → 차액 결제 → `applySlotChange` 가 `ACCEPT_PENDING` 으로 되돌린다. **강사가 제안한 슬롯인데도 재수락을 받는 이유**: 제안은 "그 시간에 가능하다"는 동의지 "차액을 받고 그 조건으로 진행한다"는 확인까지는 아니고, 제안 후 결제까지 최대 `paymentTtlHours` 가 흘러 강사 사정이 바뀔 수 있다. 강사 hub 엔 자기가 제안한 자리를 다시 확인하는 모양으로 보인다(의도됨). ⛔ 기저 원칙([[instructor-approval-required-for-scheduling]])의 "pick 예외" 는 **추가 결제 없이 즉시 확정되는 pick-slot** 에 한한다.
- **제안 hold 회수는 "제안을 비우는 모든 경로"에서(2026-08-11, 종료 경로까지 2026-08-14)** — `proposedSlots.clear()` 하는 곳은 hold 도 같이 풀어야 한다. `pickSlot` 은 원래 했지만 `swapSlot`(reschedule·supersede)·`applySlotChange`(차액 승인)는 안 해서, 학생이 제안을 안 고르고 다른 슬롯으로 가면 **아무도 못 쓰는 고아 hold** 가 남아 `proposalTtlHours`(6h) 동안 남의 신청을 막았다(정원 1이면 그 시간대가 통째로 잠김). 스윕이 결국 풀어주지만 6h 는 길다. 고정 = `M6-1`·`C1-3`. **종료 경로도 같다(H4)** — 제안받은 회차를 **학생 취소(`cancel`)·강사 거절(`reject`)** 하면 그 회차의 제안 hold 도 회수해야 한다(둘 다 `releaseProposalHolds` + `proposedSlots.clear()` 추가). 만료(`expireOne`)는 회차 24h > 제안 6h 라 스윕이 이미 풀어 무해. 고정 = `H4-1`·`H4-2`.
- **내 제안 hold 가 나를 막지 않게(2026-08-11)** — 제안 hold 는 "학생이 고르면 만석으로 안 막히게" 자리를 맡는 것인데, 비싼 제안은 차액 결제로 우회하므로 `holdSlotForOrder` 가 그 hold 를 만석으로 세어 **안내한 경로가 400 데드엔드**가 됐다(정원 1이면 확정적). `requireSeat(session, ignoreProposalRoundId)` 로 내 몫 hold 를 빼고, `applySlotChange` 는 제안 hold 를 회수한다(안 풀면 실점유와 겹쳐 좌석 이중 점유). 고정 = `C1-2`.
- **EnrollmentService 가 내는 전용 에러코드 3종** — `-1018`(더 비싸서 차액 결제 필요) · `-1019`(위치까지 바뀌며 오름 → 차액 경로 불가, **임시 가드**) · `-1020`(강사 제안 만료 → 직접 선택 유도). 셋 다 "요청은 멀쩡한데 사용자가 다음 행동을 해야 하는" 성격이라 범용 `-1011` 과 가른 것 — 새 코드를 만들 땐 이 기준을 쓴다. 계약은 `docs/api-clients/types.ts` 의 `ErrorCode`.
...
- **선결제 (2026-08-07 도입 → 2026-08-09 전 회차 통일)** — **모든 회차가 신청 즉시 결제**: 신청 `PENDING`(미결제·좌석 점유) → 결제 → `ACCEPT_PENDING`(결제완료·강사 결정 대기) → 강사 수락 `CONFIRMED` / 거절·무응답 24h `REJECTED`·`CANCELLED` + **전액 자동환불**(`EnrollmentRefundRequestedEvent`→payment `EnrollmentRefundListener`→`RefundService.refundRoundFully`, 동기·롤백안전). 미결제 12h 만료(환불 없음). 카드사 심사가 익숙한 "주문 즉시 결제" 표준으로 바꿔 심사 리스크↓ + 어차피 붙일 방향.
  ...
  - **금액 불변식** — "그 회차에 남은 결제 순액 == `chargeTotal()`". 결제 후 슬롯 변경은 줄면 `EnrollmentPartialRefundRequestedEvent`로 차액 자동환불. ~~늘면 400(취소 후 재신청)~~ → **2026-08-10 폐기**: `-1018` → 차액 결제(#204). 위치까지 바뀌면 `-1019`. 그래서 payment 조회 없이 변경 전 `chargeTotal()`이 곧 결제액이다.
