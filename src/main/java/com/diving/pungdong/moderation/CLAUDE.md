...
## 큐에 싣는 값은 "지금 아니면 못 얻는가" 로 가른다

접수 시점에 **고정**하는 것과 조회 때 **계산**하는 것의 경계다.

- **`target_author_account_id` 는 접수 때 박는다.** 대상(글·강의·메시지)이 지워지면 작성자는 되찾을
  수 없고, 그 행은 영영 "누구에 대한 신고인지 모르는 행" 이 된다. 접수는 이미 작성자를 확인하고 있다
  (자기 것 신고 차단). ⚠️ 새 대상을 더할 때 `requireTargetAuthor` 가 돌려준 값을 **저장까지** 할 것.
- **`admin_note` 도 그 순간의 정보다.** 조치가 무거워(강의면 사실상 판매 중단) 분쟁은 대개 기각으로
  끝나는데, 메모가 없으면 "따로 경고함" 과 "문제없음" 이 같은 행이 된다.
- **나머지는 계산해서 싣는다** — 작성자별 누적 신고 수 · 신고자의 그 강의 신청 이력. 원본이 딴 데
  살아 있어 언제 붙여도 소급되기 때문이다. 단 **배치로**: `QueueContext` 가 페이지 하나의 id 를 모아
  세 번만 물어본다. 행마다 조회하면 페이지 크기만큼 쿼리가 곱해진다(`targetPreview` 만 아직 행 단위 —
  대상 종류가 넷이라 배치가 안 된다).
